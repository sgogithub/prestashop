...
class Espay extends PaymentModule
{
	...
	public function __construct()
	{
	  ...
	  $this->ps_versions_compliancy = array(
        ...
        $this->bootstrap = true;
        #$this->module_key = '73e7b87edb0d4erodua7d9cb6d6b72cb377a';
		...
		$config = Configuration::getMultiple ( array (
				...
				'SGO_PAYMENT_PASS',
				'SGO_PAYMENT_IP',
				'SGO_PAYMENT_ENVIRONMENT'
		) );
		if (isset ( $config ['SGO_PAYMENT_ID'] )) {
			$this->sgopayment_id = $config ['SGO_PAYMENT_ID'];
		}
		if (isset ( $config ['SGO_PAYMENT_ENVIRONMENT'] )) {
			$this->sgopayment_environment = $config ['SGO_PAYMENT_ENVIRONMENT'];
		}
	...
	private function _postProcess()
	{
		if (Tools::isSubmit ( 'btnSubmit' )) {
			...
			Configuration::updateValue ( 'SGO_PAYMENT_ENVIRONMENT', Tools::getValue ( 'sgopaymentenvironment' ) );
	...
	private function _displayForm()
	{
		...
		<tr><td width="130" style="height: 35px;">' . $this->l ( 'Environment' ) . '</td><td><input type="radio" '.(Tools::getValue ( 'owner', $this->sgopayment_environment ) == '0' || !isset( $this->sgopayment_environment )  ? 'checked="checked"' : '').' name="sgopaymentenvironment" value="0"  /> Development <input type="radio" name="sgopaymentenvironment" '.((Tools::getValue ( 'owner', $this->sgopayment_environment ) == '1'  ) ? 'checked="checked"' : '' ).' value="1"/> Production</td></tr>
	...
	public function hookPayment($params)
	{
		...
		$this->smarty->assign ( array (
				...
				'this_path' => $this->_path,
				'enviroment' => $this->sgopayment_environment,
				'list_tab' => $this->listtab(),
				...
				if ($this->sgopayment_pass == $password) {
					...
					if ($status == 'waiting') {
						...
						try{
							...
							die('1,'.$ex->getMessage().',,' . $id_cart . ',' . date ( 'Y-m-d H:i:s' ) . '');
						}
	...
	public function serve() {
      ...
      $product = $result ['product'];
          ...
          if ($product == 'CREDITCARD'){
            $totalAll = floatval ($total) + floatval(8000) ;
            $totalFee = $totalAll * floatval(0.03);
	...
	public function confirmation() {
		...
		if ($this->_getPaymentStatus ( $cart_id ) != '') {
			...
			if ($product == 'BIIATM' || $product == 'PERMATAATM'){
				...
				$this->_updateStatuspaymen ('failed', $cart_id );
				$this->validateOrder ( $cart_id, $orderState, $total, $this->displayName, NULL, $this->_getCurrencyId ( $currency ) );
				Tools::redirect ( 'index.php?controller=order-confirmation&id_cart=' . $cart_id . '&id_module=' . $this->id . '&id_order=' . $id_order . '&key=' . $key );
			}
	...
	public function initiateProduct() {
		...
		if ($this->sgopayment_environment == '1'){
			$urlMerchant = 'https://116.90.162.172:812/rest/merchant/merchantinfo';//production
		}
	...
	private function listtab(){
		...
		foreach ($result as $row){
			$temptablist[] = $this->typePayment[$row['productcode']];
		}

		$tablist = array_unique($temptablist);
		$i =0;
		foreach ($tablist as $tab){
			$list .= ' <li '.($i === 0 ? 'class="active"' : '').'><a data-toggle="tab" href="#section'.$i.'">'.$tab.'</a></li>';
			$i++;
		}
	...
	private function listproduct(){
		...
		if ($jumlah != 0){
			...
			foreach ($tablist as $tab){
				...
				$list .= '<div id="section'.(string)$i.'" class="tab-pane fade in '.($i === 0 ? "active" : "").'" >'.PHP_EOL;
				$list .= '<h3>'.$tab.'</h3>'.PHP_EOL;
				foreach ($result as $row){
					if ($this->typePayment[$row['productcode']] == $tab){
              $list .= '<div class="col-sm-3 col-md-3 col-lg-3">
                  		<label class="thumbnail">
                  		<input type="radio" name="espayproduct" id="espayproduct" value="'.$row['bankcode'].':'.$row['productcode'].'">
                  		<img src="https://secure.sgo.co.id/images/products/'.$row['productcode'].'.png" >
                  		</label>
                  		</div>';
            #$list .= '<p><input type="radio" name="espayproduct" id="espayproduct" value="'.$row['bankcode'].':'.$row['productcode'].'"> '.$row['productname'].'</p>'.PHP_EOL;
					}
				}
				$list .= '</div>'.PHP_EOL;
				$i++;
	...
	public function hookOrderConfirmation($params){
		...
		if ($status == 'waiting'){
			$template = 'order-waiting.tpl';
		}else if($status == 'success'){
			$template = 'order-success.tpl';
		}else{
			$template = 'order-failed.tpl';
		}
